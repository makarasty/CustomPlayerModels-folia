	...
	public static class Renderer extends PictureInPictureRenderer<State> {
		...
		protected void renderToTexture(State pipState, PoseStack poseStack) {
			...
			try {
				...
				panel.render(new com.tom.cpl.math.MatrixStack(), new VBuffers(rt -> new VBuffer(bufferSource.getBuffer(rt.getNativeType()), light, OverlayTexture.NO_OVERLAY, lw, poseStack)), partialTicks);
			} catch (Exception e) {
