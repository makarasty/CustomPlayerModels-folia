...
public class ClientProxy extends CommonProxy {
	...
	public void playerRenderPreC(RenderPlayerEvent.Pre event) {
		if(event.isCanceled())manager.unbindFlush(event.getRenderer().getMainModel());
	}

	@SubscribeEvent
	public void playerRenderPost(RenderPlayerEvent.Post event) {
		manager.unbindFlush(event.getRenderer().getMainModel());
	}
	...
	public void openGui(GuiOpenEvent openGui) {
		...
		if(openGui.getGui() instanceof GuiMainMenu && !(minecraft.currentScreen instanceof GuiWorldSelection || minecraft.currentScreen instanceof GuiMainMenu) && EditorGui.doOpenEditor()) {
	...
	protected void renderLivingLabel(Entity entityIn, String str, double x, double y, double z, int maxDistance) {
		double d0 = entityIn.getDistanceSqToEntity(minecraft.getRenderManager().renderViewEntity);
		...
		if (d0 <= maxDistance * maxDistance) {
			...
			drawNameplate(minecraft.fontRendererObj, str, (float)x, (float)y + f2, (float)z, i, f, f1, flag1, flag);
		}
	}
