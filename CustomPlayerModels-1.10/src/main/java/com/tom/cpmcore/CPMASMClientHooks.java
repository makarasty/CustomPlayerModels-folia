...
public class CPMASMClientHooks {
	...
	public static void onElytraPre(LayerElytra this0, AbstractClientPlayer entitylivingbaseIn) {
	...
	public static void onElytraPost(LayerElytra this0, AbstractClientPlayer entitylivingbaseIn) {
	...
	public static boolean renderCape(LayerCape this0, AbstractClientPlayer player, float partialTicks) {
		...
		if(pl != null) {
			...
			if(def != null && def.hasRoot(RootModelType.CAPE)) {
				...
				if(!player.isInvisible() && player.isWearing(EnumPlayerModelParts.CAPE) && (chestplate == null || chestplate.getItem() != Items.ELYTRA)) {
