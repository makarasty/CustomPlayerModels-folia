...
public class ClientBase {
	...
	public static interface PlayerNameTagRenderer<E extends Entity> {
		void cpm$renderNameTag(E entityIn, String displayNameIn, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn);
	...
	public <E extends Entity> void renderNameTag(PlayerNameTagRenderer<E> r, E entityIn, GameProfile gprofile, String unique, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
		...
		if (d0 < 32*32) {
			...
			if(st != null) {
				...
				r.cpm$renderNameTag(entityIn, st.<ITextComponent>remap().getColoredString(), matrixStackIn, bufferIn, packedLightIn);
				matrixStackIn.popPose();
			}
		}
	}
