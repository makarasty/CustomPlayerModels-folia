...
public class PlayerProfile extends Player<PlayerEntity> {
	...
	public void updateFromPlayer(AnimationState animState, PlayerEntity player) {
		...
		animState.sneaking = player.isVisuallySneaking();
		...
		animState.moveAmountX = (float) (player.x - player.xo);
		animState.moveAmountY = (float) (player.y - player.yo);
		animState.moveAmountZ = (float) (player.z - player.zo);
		...
		animState.isOnLadder = player.onLadder();
				...
				Minecraft.getInstance().getSkinManager().registerSkins(profile, new ISkinAvailableCallback() {
					...
					public void onSkinTextureAvailable(Type typeIn, ResourceLocation location, MinecraftProfileTexture profileTexture) {
						...
						case SKIN:
							...
							CustomPlayerModelsClient.mc.getDefinitionLoader().execute(() -> Minecraft.getInstance().execute(() -> cf.complete(null)));
							break;
