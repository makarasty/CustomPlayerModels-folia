	...
	public static void init() {
		...
		transformers.put("net.minecraft.client.renderer.entity.layers.LayerArmorBase", new UnaryOperator<ClassNode>() {
			...
			public ClassNode apply(ClassNode input) {
				...
				for(MethodNode method : input.methods) {
					...
					} else if((method.name.equals("a") && method.desc.equals("(Lvp;FFFFFFF)V")) || method.name.equals("doRenderLayer")) {
					...
					} else if((method.name.equals("a") && method.desc.equals("(Lcaa;Lvp;Lbqf;FFFFFFF)V")) || method.name.equals("renderEnchantedGlint")) {
		...
		transformers.put("net.minecraft.client.renderer.entity.RenderPlayer", new UnaryOperator<ClassNode>() {
			...
			public ClassNode apply(ClassNode input) {
				...
				LOG.info("CPM Render Invis Hook/Is Visible: injected");
		...
		transformers.put("net.minecraft.client.renderer.entity.layers.LayerElytra", new UnaryOperator<ClassNode>() {
			...
			public ClassNode apply(ClassNode input) {
				for(MethodNode method : input.methods) {
					if((method.name.equals("a") && method.desc.equals("(Lvp;FFFFFFF)V")) || method.name.equals("doRenderLayer")) {
						LOG.info("CPM Elytra Hook/Bind: Found doRenderLayer method");
						Type[] argsD = Type.getArgumentTypes(method.desc);
						Type[] args = new Type[2];
						args[0] = Type.getObjectType(input.name);
						args[1] = argsD[0];
						for (ListIterator<AbstractInsnNode> it = method.instructions.iterator(); it.hasNext(); ) {
							...
							if(insnNode instanceof MethodInsnNode) {
								...
								if(mn.desc.equals("()V") && (mn.name.equals("pushMatrix") || (mn.name.equals("G") && mn.owner.equals("bus")))) {
									LOG.info("CPM Elytra Hook/Bind pre: Found pushMatrix method");
									InsnList lst = new InsnList();
									lst.add(new VarInsnNode(Opcodes.ALOAD, 0));
									lst.add(new VarInsnNode(Opcodes.ALOAD, 1));
									lst.add(new MethodInsnNode(Opcodes.INVOKESTATIC, HOOKS_CLASS, "onElytraPre", Type.getMethodDescriptor(Type.VOID_TYPE, args), false));
									method.instructions.insert(mn, lst);
									LOG.info("CPM Elytra Hook/Bind pre: injected");
								} else if(mn.desc.equals("()V") && (mn.name.equals("popMatrix") || (mn.name.equals("H") && mn.owner.equals("bus")))) {
									LOG.info("CPM Elytra Hook/Bind post: Found popMatrix method");
									InsnList lst = new InsnList();
									lst.add(new VarInsnNode(Opcodes.ALOAD, 0));
									lst.add(new VarInsnNode(Opcodes.ALOAD, 1));
									lst.add(new MethodInsnNode(Opcodes.INVOKESTATIC, HOOKS_CLASS, "onElytraPost", Type.getMethodDescriptor(Type.VOID_TYPE, args), false));
									method.instructions.insert(mn, lst);
									LOG.info("CPM Elytra Hook/Bind post: injected");
								}
							}
						}
					}
