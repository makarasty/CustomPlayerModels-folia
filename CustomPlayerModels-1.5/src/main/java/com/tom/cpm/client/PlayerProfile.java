...
public class PlayerProfile extends Player<EntityPlayer> implements IPlayerProfile {
	...
	public void updateFromPlayer(AnimationState animState, EntityPlayer player) {
		...
		ItemStack is = player.getCurrentItemOrArmor(4);
		animState.hasSkullOnHead = is != null && is.getItem() instanceof ItemSkull;
		animState.wearingHelm = is != null;
		animState.wearingBody = player.getCurrentItemOrArmor(3) != null;
		animState.wearingLegs = player.getCurrentItemOrArmor(2) != null;
		animState.wearingBoots = player.getCurrentItemOrArmor(1) != null;
