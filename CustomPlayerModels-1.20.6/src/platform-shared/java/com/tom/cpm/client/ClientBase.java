...
public abstract class ClientBase {
	...
	public void init1() {
		...
		netHandler = new NetHandler<>((k, v) -> new CustomPacketPayload.Type<>(new ResourceLocation(k, v)));
	...
	public static interface PlayerNameTagRenderer<E extends Entity> {
		void cpm$renderNameTag(E entityIn, Component displayNameIn, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, float f0);
		EntityRenderDispatcher cpm$entityRenderDispatcher();
	}

	public <E extends Entity> void renderNameTag(PlayerNameTagRenderer<E> r, E entityIn, GameProfile gprofile, String unique, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, float f0) {
		...
		if (d0 < 100.0D) {
			...
			if(st != null) {
				...
				r.cpm$renderNameTag(entityIn, st.remap(), matrixStackIn, bufferIn, packedLightIn, f0);
