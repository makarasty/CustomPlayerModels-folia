...
public abstract class BrigadierCommandHandlerBase<S> implements CommandHandler<S> {
	...
	public BrigadierCommandHandlerBase() {
	}

	protected void build(ArgumentBuilder<S, ?> a, AbstractCommandBuilder<?> b, List<RequiredCommandBuilder> args, String prefix) {
	...
	private class WrappedCtx extends CommandCtx<S> {
		...
		public WrappedCtx(CommandContext<S> ctx, List<RequiredCommandBuilder> args) throws CommandSyntaxException {
			super(ctx.getSource(), BrigadierCommandHandlerBase.this);
