...
public class KeyBindings implements IKeybind {
	...
	public static void init(RegisterKeyMappingsEvent evt) {
		...
		gestureMenuBinding = new KeyMapping("key.cpm.gestureMenu", KeyConflictContext.IN_GAME, InputConstants.Type.KEYSYM.getOrCreate(GLFW.GLFW_KEY_H), category, 0);
		renderToggleBinding = new KeyMapping("key.cpm.renderToggle", KeyConflictContext.IN_GAME, InputConstants.UNKNOWN, category, 1);
	...
	private static class KeyConflictCtx implements IKeyConflictContext {
		...
		public boolean isActive() {
			...
			return mc.gui.screen() instanceof GuiImpl || (!KeyConflictContext.GUI.isActive() && mc.player != null);
		}
	...
	private static void createQA(int id, RegisterKeyMappingsEvent evt) {
		KeyMapping kb = new KeyMapping("key.cpm.qa_" + id, conflictCtx, InputConstants.UNKNOWN, category, id + 2);
