...
public class Panel3dImpl extends Panel3dNative {
	...
	public static class Renderer extends PictureInPictureRenderer<State> {
		...
		protected void renderToTexture(State pipState, PoseStack poseStack,
			...
			try {
				...
				int light = 15728880;
				float lw = mc.getWindow().getAppropriateLineWidth();

				RenderCollector collector = new RenderCollector(poseStack, submitNodeCollector, null, light, OverlayTexture.NO_OVERLAY, -1, 0, null, lw, null);
				VBuffers bufs = VBuffers.record(collector.recordBuffer());
				panel.render(new MatrixStack(), bufs, partialTicks);
				bufs.finishAll();
			} catch (Exception e) {
	...
	public RenderTypes<RenderMode> getRenderTypes(String texture) {
		return getRenderTypes0(Identifier.tryBuild("cpm", "textures/gui/" + texture + ".png"));
	}
