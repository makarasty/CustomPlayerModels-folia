...
public class CPMOrderedSubmitNodeCollector implements OrderedSubmitNodeCollector {
	...
	public <S> void submitModel(Model<? super S> model, S state, PoseStack pose, RenderType type,
		...
		if (model.root() instanceof SelfRenderer sr) {
			...
			sr.submitSelf(new RenderCollector(pose, collector, type, light, overlay, tint, outline, sprite, 0f, state));
		} else
	...
	public <S> void submitModel(Model<? super S> model, S state, PoseStack pose, RenderType type,
		...
		if (model.root() instanceof SelfRenderer sr) {
			model.setupAnim(state);
			sr.submitSelf(new RenderCollector(pose, collector, type, light, overlay, -1, outline, null, 0f, state));
		} else
	...
	public void submitModelPart(ModelPart part, PoseStack pose, RenderType type, int light,
		...
		if (part instanceof SelfRenderer sr) {
			sr.submitSelf(new RenderCollector(pose, collector, type, light, overlay, -1, 0, sprite, 0f, null));
		} else
	...
	public void submitModelPart(ModelPart part, PoseStack pose, RenderType type, int light,
		...
		if (part instanceof SelfRenderer sr) {
			sr.submitSelf(new RenderCollector(pose, collector, type, light, overlay, tint, 0, sprite, 0f, null));
		} else
	...
	@Override
	public void submitModelPart(ModelPart part, PoseStack pose, RenderType type, int light,
			int overlay, @Nullable TextureAtlasSprite sprite, int tint,
			@Nullable CrumblingOverlay crumblingOverlay, int outlineColor) {
		if (part instanceof SelfRenderer sr) {
			sr.submitSelf(new RenderCollector(pose, collector, type, light, overlay, tint, outlineColor, sprite, 0f, null));
		} else
			collector.submitModelPart(part, pose, type, light, overlay, sprite, tint,
