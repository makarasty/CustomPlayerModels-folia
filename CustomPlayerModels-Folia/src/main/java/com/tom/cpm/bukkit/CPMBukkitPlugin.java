...
public class CPMBukkitPlugin extends JavaPlugin {
    private static CPMBukkitPlugin instance;
    ...
    public static CPMBukkitPlugin getInstance() {
        return instance;
    }
    ...
    public void onEnable() {
        ...
        MinecraftObjectHolder.setCommonObject(new MinecraftCommonAccess() {
            @Override public ModConfigFile getConfig() { return config; }
            @Override public ILogger getLogger() { return log; }
            @Override public EnumSet<PlatformFeature> getSupportedFeatures() { return EnumSet.noneOf(PlatformFeature.class); }
            @Override public String getPlatformVersionString() { return "Bukkit (" + getServer().getVersion() + "/" + getServer().getBukkitVersion() + ") " + getDescription().getVersion(); }
            @Override public TextRemapper<?> getTextRemapper() { return cmd.remapper(); }
            @Override public CPMApiManager getApi() { return api; }
            @Override public String getMCVersion() { return "bukkit"; }
            @Override public String getMCBrand() { return "Bukkit (" + getServer().getVersion() + "/" + getServer().getBukkitVersion() + ")"; }
            @Override public String getModVersion() { return getDescription().getVersion(); }
            @Override public ItemStackHandler<?> getItemStackHandler() { return null; }
            @Override public BlockStateHandler<?> getBlockStateHandler() { return null; }
            @Override public EntityTypeHandler<?> getEntityTypeHandler() { return null; }
        });
        MinecraftObjectHolder.setServerObject(new MinecraftServerAccess() {
            @Override public ModConfigFile getConfig() { return config; }
            @Override public NetHandler<?, ?, ?> getNetHandler() { return net.netHandler; }
            @Override public BiomeHandler<?> getBiomeHandler() { return null; }
        });
        net = new Network(this);
        net.register();
        log.info("Customizable Player Models Initialized for Folia");
    }
