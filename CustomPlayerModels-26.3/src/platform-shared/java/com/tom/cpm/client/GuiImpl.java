...
import com.tom.cpm.client.GuiGraphicsExtractorEx.StateFactory;
import com.tom.cpm.client.MinecraftObject.DynTexture;
import com.tom.cpm.common.ItemStackHandlerImpl;
import com.tom.cpm.mixin.access.HudAccessor;
...
public class GuiImpl extends Screen implements IGui {
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
		...
		if(minecraft.player != null && gui.enableChat()) {
			((HudAccessor) minecraft.gui.hud).callExtractChat(graphics, minecraft.getDeltaTracker());
		}
	}
	...
	public void drawBox(float xI, float yI, float w, float h, int color) {
		...
		((GuiGraphicsExtractorEx) graphics).cpm$fill(x, y, x + w, y + h, color);
	}
			...
			if(!evt.isConsumed()) {
				if(minecraft.player != null && minecraft.options.keyChat.matches(keyEvent) && minecraft.options.chatVisibility().get() != ChatVisiblity.HIDDEN) {
					Minecraft.getInstance().schedule(() -> {
						...
						vanillaScale = -1;
						minecraft.gui.hud.getChat().openScreen(ChatComponent.ChatMethod.MESSAGE, (a, b) -> new Overlay(scale, a, b));
						vanillaScale = scale;
					});
					return true;
				}
	...
	public void displayError(String e) {
		Screen p = parent;
		parent = null;
		Minecraft.getInstance().setScreenAndShow(new CrashScreen(e, p));
	}
	...
	private static class CrashScreen extends Screen {
		...
		public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
			...
			for (int i = 0; i < txt.length; i++) {
				graphics.centeredText(this.font, txt[i], this.width / 2, 15 + i * 10, 0xFFFFFFFF);
			}
		}
		...
		protected void init() {
			super.init();
			this.addRenderableWidget(Button.builder(CommonComponents.GUI_BACK, _ -> {
	...
	private class TxtField implements ITextField, Consumer<String> {
		...
		public void draw(int mouseX, int mouseY, float partialTicks, Box bounds) {
			...
			field.extractRenderState(graphics, mouseX, mouseY, partialTicks);
		}
		...
		public void keyPressed(KeyboardEvent evt) {
			...
			if(evt.keyCode == -1) {
				if(field.charTyped(new CharacterEvent(evt.charTyped)))
					evt.consume();
			} else {
	...
	public class Overlay extends ChatScreen {
		...
		public Overlay(int vanillaScaleReset, final String initial, final boolean isDraft) {
			super(initial, isDraft);
			this.vanillaScaleReset = vanillaScaleReset;
		}

		@Override
		public void extractRenderState(GuiGraphicsExtractor gr, int mouseX, int mouseY, float partialTicks) {
			GuiImpl.this.extractRenderState(gr, Integer.MIN_VALUE, Integer.MIN_VALUE, partialTicks);
			super.extractRenderState(gr, mouseX, mouseY, partialTicks);
		}
		...
		public void removed() {
			super.removed();
			minecraft.schedule(() -> {
				minecraft.setScreenAndShow(GuiImpl.this);
	...
	public void drawGradientBox(int xI, int yI, int w, int h, int topLeft, int topRight, int bottomLeft,
		...
		((GuiGraphicsExtractorEx) graphics).cpm$fillGradient(x, y, x + w, y + h, topLeft, topRight, bottomLeft, bottomRight);
	}
	...
	public void drawFormattedText(float x, float y, IText text, int color, float scale) {
		...
		graphics.text(font, text.<Component>remap(), 0, 0, color | 0xFF000000, false);
		graphics.pose().popMatrix();
	}
	...
	public void drawStack(int x, int y, Stack stack) {
		...
		graphics.item(s, x, y);
		graphics.itemDecorations(font, s, x, y);
	}
	...
	public <I, S extends PictureInPictureRenderState> void drawPip(I instance, int w, int h, StateFactory<I, S> sf) {
		int x = getOffset().x;
		int y = getOffset().y;
		((GuiGraphicsExtractorEx) graphics).cpm$pip(instance, x, y, x + w, y + h, sf);
	}
